	...
	<div class="dashboard">
		...
		<main class="container">
			...
			<div v-if="!isLoading && stats" class="stats-grid">
				...
				<div class="stat-card">
					<h3>국가 수</h3>
					<div class="value">{{ formatNumber(stats.totalCountries) }}</div>
				</div>
				<div class="stat-card">
					<h3>대륙 수</h3>
					<div class="value">{{ formatNumber(stats.continentCount) }}</div>
				</div>
			</div>
			...
			<section v-if="!isLoading && continentStats.length > 0" class="section">
				...
				<div class="table-wrapper">
					<table>
						...
						<tbody>
							<tr v-for="stat in continentStats" :key="stat.continent" @click="goToContinent(stat.continent)" style="cursor: pointer">
								<td>
			...
			<section v-if="!isLoading && topCountries.length > 0" class="section">
				...
				<div class="table-wrapper">
					<table>
						...
						<tbody>
							<tr v-for="(country, index) in topCountries" :key="country.countryCode" @click="goToCountry(country.countryCode)" style="cursor: pointer">
								<td>{{ index + 1 }}</td>
								<td>
			...
			<section class="section">
				...
				<div class="search-box">
					<form @submit.prevent="handleSearch">
						<input v-model="searchKeyword" type="text" placeholder="국가명 검색..." @keyup="onSearchInput" />
					</form>
				</div>
				...
				<div class="table-wrapper">
					<table>
						...
						<tbody>
							<tr v-for="country in countries" :key="country.countryCode" @click="goToCountry(country.countryCode)" style="cursor: pointer">
								...
								<td>{{ formatDecimal(country.lifeExpectancy, 2) }}</td>
							</tr>
						</tbody>
					</table>
				</div>
				...
				<div v-if="pagination.totalPages > 1" class="pagination">
					<button v-if="pagination.hasPreviousPage" @click="previousPage" class="page-btn">이전</button>

					<button v-for="page in pagination.pageNumbers" :key="page" @click="goToPage(page)" :class="['page-btn', { active: page === pagination.currentPage }]">
						{{ page }}
					</button>

					<button v-if="pagination.hasNextPage" @click="nextPage" class="page-btn">다음</button>
				</div>
			</section>
		</main>
	</div>
...
import { formatCurrency, formatDecimal, formatNumber, getErrorMessage, getFlagUrl, logout } from "../utils/helpers";
...
const pagination = computed(() => {
	const pages = Array.from({ length: Math.min(5, totalPages.value) }, (_, i) => {
